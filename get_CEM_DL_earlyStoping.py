...
class TQDMProgressBar(callbacks.Callback):
    def on_epoch_begin(self, epoch, logs=None):
        # Initialiser la barre de progression avant chaque époque
        self.pbar = tqdm(total=self.params['epochs'], desc="Training", unit="epoch")
        
    def on_epoch_end(self, epoch, logs=None):
        # Mettre à jour la barre de progression à la fin de chaque époque
        self.pbar.set_postfix(logs)
        self.pbar.update(1)
        
    def on_train_end(self, logs=None):
        # Fermer la barre de progression à la fin de l'entraînement
        self.pbar.close()
